...
- 연습문제 풀이는 공식 정답이 아닌 학습 과정의 노트로 작성한다. 저작권 보호를 위해 문제 원문은 그대로 옮기지 않고 번호와 제목만 표기하며, 핵심 아이디어와 풀이 과정을 한국어로 서술한다. 필요한 경우 원문의 핵심 문장을 인용할 수 있다.
...
- GitHub에는 학습 노트와 학습·평가 코드를 둔다.
- 재학습 비용이 크거나 재사용할 가치가 있는 학습 모델 산출물은 Hugging Face Hub에 배포한다. 작은 밴딧·표 형식 실험은 모델을 별도로 배포하지 않고 재현 가능한 코드만 둘 수 있다.
- 모델 가중치에는 환경과 버전, 관측·행동 정의, 전처리·정규화 설정과 통계, 학습 조건·시드·스텝 수, 평가 조건과 결과, 코드 커밋, 불러오기·실행 예시를 모델 카드나 설정 파일로 함께 제공한다.
- 공개 업로드는 사용자가 명시적으로 요청한 경우에만 수행한다.

## 변경 검증

- 산문만 변경했다면 내용과 문서 형식을 검토하고, 전체 노트북 실행은 생략할 수 있다.
- `notes.md`나 `solutions.md`의 내용이 `notes.ipynb`에도 들어가는 구성이라면 관련 마크다운 셀과의 일관성을 확인한다.
- 노트북의 코드 셀을 변경했다면 필요한 임포트와 데이터 생성 등을 포함한 상태로 해당 셀을 독립 실행해 결과를 확인한다.
- 변경한 마크다운과 노트북의 형식을 검사하고, 커밋이 승인된 경우에는 커밋 전에 `pre-commit run --files <변경 파일>`을 실행한다.
